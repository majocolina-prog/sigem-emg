...
function setupPatientForm() {
  $("#patient-form").addEventListener("submit", (event) => {
    ...
    const patient = {
      ...
      location: data.location.trim() || "No consignada",
      responsible: data.responsible.trim() || getCurrentUser().name,
      initial: null,
      evolutions: [],
      orders: [],
...
function renderDashboard() {
  ...
  const patients = state.patients.filter((patient) => {
    const matchesSearch = [patient.fullName, patient.documentId, patient.clinicalRecord, patient.episodeNumber, patient.chiefComplaint]
      .join(" ")
      .toLowerCase()
      .includes(search);
...
function renderAdmissionTab(patient) {
  ...
  return `
    <form class="entry-form" data-admission-form>
      <label>Nombre y apellido <input required name="fullName" value="${escapeHtml(patient.fullName || "")}" /></label>
      ...
      <label>Ubicación inicial / actual <input name="location" value="${escapeHtml(patient.location || "")}" /></label>
      <label>Responsable inicial <input name="responsible" value="${escapeHtml(patient.responsible || "")}" /></label>
...
function renderInitialTab(patient) {
  ...
  return `
    <form class="entry-form" data-clinical-form="initial">
      ...
      <label class="full">Resumen operativo para enfermería <textarea name="nursingSummary">${escapeHtml(data.nursingSummary || "")}</textarea></label>
...
function saveAdmission(patientId, data) {
  ...
  Object.assign(patient, {
    ...
    status: data.status,
    location: data.location.trim() || "No consignada",
    responsible: data.responsible.trim() || patient.responsible || getCurrentUser().name,
    updatedAt: timestamp,
  });
